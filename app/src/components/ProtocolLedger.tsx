...
const ProtocolLedger: React.FC<ProtocolLedgerProps> = ({ onClose, highlightHash }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-4 animate-fade-in font-sans">
            <div className="w-full max-w-4xl h-[90vh] bg-slate-900/50 rounded-3xl border border-slate-800 relative overflow-hidden flex flex-col shadow-2xl">
                ...
                {/* Header: User Transaction */}
                <div className="p-6 border-b border-slate-800 bg-slate-900/80 backdrop-blur z-20 flex justify-between items-start">
                    <div>
                        <div className="text-[10px] uppercase font-bold text-slate-500 tracking-widest mb-1">Incoming Transaction</div>
                        <div className="font-mono text-xl text-white flex items-center gap-2">
                            <span className="text-emerald-400">3.00 CRD</span>
                            <span className="text-slate-600">→</span>
                            <span className="opacity-75">{highlightHash ? `${highlightHash.substring(0, 12)}...` : '0x...'}</span>
                        </div>
                    </div>
                ...
                <div className="flex-1 relative overflow-hidden flex flex-col items-center justify-center py-10">
                    ...
                    {/* Emitter Point */}
                    <div className="absolute top-0 w-1 h-10 bg-gradient-to-b from-emerald-500/50 to-transparent"></div>

                    {/* Flow Lines Background */}
                    <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none">
                        <path d="M50% 0 L 50% 20% L 20% 50%" fill="none" stroke="#10B981" strokeWidth="2" />
                    ...
                    <div className="w-full grid grid-cols-3 gap-6 px-12 relative z-10 mt-10">
                        {/* 1. Security Vault */}
                        <div className="bg-slate-800/80 border border-emerald-500/20 rounded-2xl p-6 relative overflow-hidden group">
                            <div className="absolute inset-0 bg-emerald-500/5 group-hover:bg-emerald-500/10 transition-colors"></div>
                            <div className="text-4xl mb-4">🏦</div>
                            <div className="text-3xl font-bold text-white mb-1">60%</div>
                            <div className="text-xs font-bold text-emerald-400 uppercase tracking-wider mb-2">Security Vault</div>
                            <p className="text-[10px] text-slate-400 leading-relaxed">
                                Permanent insurance backing for protocol solvency. Funds are legally locked and auditable.
                            </p>
                        </div>

                        {/* 2. Scout War Chest (Middle/Bottom visually via grid but logic is same) */}
                        <div className="bg-slate-800/80 border border-indigo-500/20 rounded-2xl p-6 relative overflow-hidden group mt-12">
                        ...
                        {/* 3. Sheriff Yield */}
                        <div className="bg-slate-800/80 border border-amber-500/20 rounded-2xl p-6 relative overflow-hidden group">
                ...
                <div className="bg-slate-900 border-t border-slate-800 p-6">
                    ...
                    <div className="grid grid-cols-4 gap-4 text-xs font-mono border border-slate-700/50 rounded-lg p-4 bg-black/20">
                        ...
                        <div className="text-emerald-400 font-bold">Protocol Vault</div>
                        <div className="text-right text-white">60%</div>
                        <div className="text-right text-white">1.80 CRD</div>
                        <div className="text-right text-emerald-400">0.003 ETH</div>

                        <div className="text-amber-400 font-bold">Sheriff Yield</div>
                        <div className="text-right text-white">25%</div>
                        <div className="text-right text-white">0.75 CRD</div>
                        <div className="text-right text-amber-400">0.00125 ETH</div>

                        <div className="text-indigo-400 font-bold">War Chest</div>
                        <div className="text-right text-white">15%</div>
                        <div className="text-right text-white">0.45 CRD</div>
                        <div className="text-right text-indigo-400">0.00075 ETH</div>
                    </div>
                </div>
