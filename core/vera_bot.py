...
async def explain_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🧠 *Sovereign Analysis* of `{address}`...", parse_mode='Markdown')
...
async def revenue_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"• Total Carry: `${stats['total_carry_usd']:.2f}`\n"
        f"• Voucher Liability: `${stats['active_vouchers_usd']:.2f}`\n"
        f"• Vault Balance: `{stats['vault_balance_eth']:.4f} ETH`\n"
        f"• Neurons Active: `{stats['neurons_active']}`"
    )
...
if __name__ == '__main__':
    ...
    async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        await context.bot.send_message(chat_id=update.effective_chat.id, text="🛡️ **Vera-Verify Active**\nSecure communication channel established.")
    ...
    async def link_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if success:
            await update.message.reply_text(f"✅ Account Linked! User ID: `{result}`", parse_mode='Markdown')
        else:
            await update.message.reply_text(f"❌ Link Failed: {result}")
    ...
    async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(text) == 42 and text.startswith("0x"):
            ...
            await update.message.reply_text(f"🔍 Analyzing Contract: `{text}`...", parse_mode='Markdown')
